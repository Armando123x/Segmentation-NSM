    ...
    def __init__(self, config_file: dict, processor: Samprocessor, mode: str):
        ...
        if mode == "train":
            ...
            for img_path in self.img_files:
                self.mask_files.append(os.path.join(config_file["DATASET"]["TRAIN_PATH"],'masks', f"{os.path.basename(img_path)}")) 

        else:
            self.img_files = glob.glob(os.path.join(config_file["DATASET"]["TEST_PATH"],'images','*.png'))
            self.mask_files = []
            for img_path in self.img_files:
                self.mask_files.append(os.path.join(config_file["DATASET"]["TEST_PATH"],'masks', f"{os.path.basename(img_path)}" ))

        self.processor = processor

    def __len__(self):
        return len(self.img_files)
 # def __getitem__(self, index: int) -> list: img_path = self.img_files[index] mask_path = self.mask_files[index] # get image and mask in PIL format image = Image.open(img_path).convert("RGB") mask = Image.open(mask_path) mask = mask.convert('1') ground_truth_mask = np.array(mask) original_size = tuple(image.size)[::-1] # get bounding box prompt box = utils.get_bounding_box(ground_truth_mask) inputs = self.processor(image, original_size, box) inputs["ground_truth_mask"] = torch.from_numpy(ground_truth_mask) return inputs
    ...
    def __getitem__(self, index):
        ...
        boxes_mask = utils.get_bounding_box(ground_truth_mask)
    
        samples = []
        for item in boxes_mask:
            box = item[0]
            mask = item[1]
