...
class Samprocessor:
    ...
    def process_prompt(self, box: list, original_size: tuple) -> torch.tensor:
        ...
        # We only use boxes
        box_torch = None
        nd_box = np.array(box)#before reshape (1,4)
        
        #### voy a comentar las lineas como backup
        #### permitirá conectar con el predict_pytorch
        #### Al fin de que pueda tener varios boxes a la vez 


        ### cambiar a cuda si en algun momento se usa cpu
        ##input_boxes = torch.tensor(box, device='cuda')  
        ###transformed_boxes = self.transform.apply_boxes_torch(input_boxes, original_size)  
