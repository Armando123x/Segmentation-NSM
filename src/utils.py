...
def get_bounding_box(ground_truth_map: np.array)  :
  """
  ...
  #######################################
  ##### Se va a modificar para que acepte multiples boundinbox

  """
  # get bounding box from mask
  labeled_mask, num_objects = label(ground_truth_map>0)
  if num_objects == 0:
      return []
  bbox_list = list()
  ...
  for obj_id in range(1, num_objects + 1):
      ...
      if abs(xs.max()-xs.min())<3:
            continue
      
      bbox_list.append((bbox, obj_mask.astype(np.uint8)))
